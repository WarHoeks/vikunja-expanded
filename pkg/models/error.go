...
const ErrCodeTaskLinkDoesNotExist = 20004
...
// ErrTaskLinkHasNoCustomIcon represents an error where a task link's custom icon is requested but none was uploaded
type ErrTaskLinkHasNoCustomIcon struct {
	ID int64
}

// IsErrTaskLinkHasNoCustomIcon checks if an error is ErrTaskLinkHasNoCustomIcon.
func IsErrTaskLinkHasNoCustomIcon(err error) bool {
	_, ok := err.(ErrTaskLinkHasNoCustomIcon)
	return ok
}

func (err ErrTaskLinkHasNoCustomIcon) Error() string {
	return fmt.Sprintf("Task link has no custom icon [ID: %d]", err.ID)
}

// ErrCodeTaskLinkHasNoCustomIcon holds the unique world-error code of this error
const ErrCodeTaskLinkHasNoCustomIcon = 20005

// HTTPError holds the http error description
func (err ErrTaskLinkHasNoCustomIcon) HTTPError() web.HTTPError {
	return web.HTTPError{
		HTTPCode: http.StatusNotFound,
		Code:     ErrCodeTaskLinkHasNoCustomIcon,
		Message:  "This task link has no custom icon.",
	}
}

// ErrTaskLinkIconIsTooLarge represents an error where an uploaded task link icon exceeds the configured file size
type ErrTaskLinkIconIsTooLarge struct {
	Size uint64
}

// IsErrTaskLinkIconIsTooLarge checks if an error is ErrTaskLinkIconIsTooLarge.
func IsErrTaskLinkIconIsTooLarge(err error) bool {
	_, ok := err.(ErrTaskLinkIconIsTooLarge)
	return ok
}

func (err ErrTaskLinkIconIsTooLarge) Error() string {
	return fmt.Sprintf("Task link icon is too large [Size: %d]", err.Size)
}

// ErrCodeTaskLinkIconIsTooLarge holds the unique world-error code of this error
const ErrCodeTaskLinkIconIsTooLarge = 20006

// HTTPError holds the http error description
func (err ErrTaskLinkIconIsTooLarge) HTTPError() web.HTTPError {
	return web.HTTPError{
		HTTPCode: http.StatusBadRequest,
		Code:     ErrCodeTaskLinkIconIsTooLarge,
